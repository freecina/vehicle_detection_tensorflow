...
def draw_bounding_box_on_image_array(current_frame_number, image,
    ...
    image_pil = Image.fromarray(np.uint8(image)).convert('RGB')
    # vehicle_counting_temp_list = [0]
    vehicle_counting_temp_list, csv_line, update_csv = draw_bounding_box_on_image(current_frame_number, image_pil, ymin,
                                                                                  xmin, ymax, xmax, color,
...
def draw_bounding_box_on_image(current_frame_number, image,
    ...
    if (bottom > 200):
        direction, speed, vehicle_counting_temp_list, update_csv = speed_prediction(top, bottom, current_frame_number,
                                                                                    right, left, crop_img)
...
def speed_prediction(top, bottom, current_frame_number, right, left, crop_img):
    ...
    if ((len(bottom_position_of_detected_vehicle_temp_list) != 0) and bottom -
            bottom_position_of_detected_vehicle_temp_list[0] > 0 and 205 <
            bottom_position_of_detected_vehicle_temp_list[0] and bottom_position_of_detected_vehicle_temp_list[
                0] < 210 and 200 < bottom):
        print("COUNTER++")
